<template>
  ...
  <main class="flex-1 overflow-y-auto scroll-smooth custom-scrollbar">
    <div class="max-w-[1400px] mx-auto px-6 lg:px-12 py-8">
        ...
        <div class="flex items-center justify-between">
          <div>
            <div class="flex items-center gap-3 mb-1">
              <div
                class="w-9 h-9 rounded-full bg-indigo-500 flex items-center justify-center text-white font-bold text-sm shadow-md">
                S</div>
              <div>
      ...
      <!-- Stats Row -->
      <section
        class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <section v-if="activeTab === 'resumes'" class="animate-in fade-in duration-300">
        ...
        <div v-else class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-5">
          <div v-for="resume in myResumes" :key="resume.id"
            ...
            <div class="relative aspect-[3/4] overflow-hidden">
              <img :src="resume.src" :alt="resume.label"
              ...
              <div
                ...
                <button
                  class="w-9 h-9 rounded-xl bg-white/20 hover:bg-white/30 backdrop-blur-sm flex items-center justify-center text-white transition-all"
                  title="Download">
                ...
                <button @click.stop="deleteItem('resume', resume.id)"
                  class="w-9 h-9 rounded-xl bg-red-500/70 hover:bg-red-500 backdrop-blur-sm flex items-center justify-center text-white transition-all"
                  title="Delete">
      ...
      <section v-if="activeTab === 'portfolios'" class="animate-in fade-in duration-300">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          <div v-for="portfolio in myPortfolios" :key="portfolio.id"
            ...
            <div class="relative aspect-[16/9] overflow-hidden">
              <img :src="portfolio.src" :alt="portfolio.label"
                class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
              <div
                class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2">
                ...
                <button @click.stop="deleteItem('portfolio', portfolio.id)"
                  class="w-9 h-9 rounded-xl bg-red-500/70 hover:bg-red-500 backdrop-blur-sm flex items-center justify-center text-white transition-all"
                  title="Delete">
                  <i class="fas fa-trash text-xs"></i>
                </button>
            ...
            <div class="p-4">
              ...
              <div v-if="portfolio.deployed" class="mt-2 flex items-center gap-1.5">
                ...
                <a :href="portfolio.url" target="_blank"
                  class="text-[10px] text-purple-500 hover:text-purple-700 transition-colors truncate font-medium">
                  {{ portfolio.url }}
                </a>
              </div>
    ...
    <Transition name="modal">
      <div v-if="showDeployModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-8 animate-in zoom-in-95 duration-200">
          ...
          <p class="text-sm text-gray-500 mb-6">Choose where to deploy <span class="font-semibold text-gray-700">{{
            deployingPortfolio?.label }}</span>.</p>
          ...
          <div class="space-y-3 mb-6">
            <label class="flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all"
              ...
              <div
                :class="['w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0', deployOption === 'subdomain' ? 'border-purple-500' : 'border-gray-300']">
              ...
              <span
                class="ml-auto text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-green-100 text-green-600">Free</span>
            </label>
            ...
            <label class="flex items-center gap-3 p-4 rounded-2xl border-2 cursor-pointer transition-all"
              ...
              <div
                :class="['w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0', deployOption === 'custom' ? 'border-purple-500' : 'border-gray-300']">
              ...
              <span
                class="ml-auto text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-600">Pro</span>
            </label>
          </div>
          ...
          <div v-if="deployOption === 'subdomain'" class="mb-6">
            ...
            <div
              class="flex items-center rounded-2xl border border-gray-200 overflow-hidden focus-within:border-purple-400 focus-within:ring-2 focus-within:ring-purple-100 transition-all">
              <input v-model="subdomainInput" type="text" placeholder="yourname"
                class="flex-1 px-4 py-3 text-sm outline-none bg-white" />
              <span
                class="px-3 py-3 text-sm text-gray-400 bg-gray-50 border-l border-gray-200 whitespace-nowrap">.porichoy.app</span>
            </div>
          </div>

          <div v-if="deployOption === 'custom'" class="mb-6">
            <label class="text-xs font-bold uppercase tracking-wider text-gray-500 mb-2 block">Custom Domain /
              URL</label>
            ...
            <p class="text-[10px] text-gray-400 mt-2">Point your domain's CNAME to <span
                class="font-mono text-purple-500">cname.porichoy.app</span></p>
          </div>
