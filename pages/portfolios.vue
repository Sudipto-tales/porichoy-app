  ...
  <main class="flex-1 overflow-y-auto scroll-smooth custom-scrollbar">
      ...
      <div class="max-w-[1600px] mx-auto px-6 lg:px-12 py-16 relative z-10">
        <div class="max-w-3xl">
          ...
          <div class="relative max-w-2xl group">
            <div class="absolute inset-0 bg-indigo-600/5 blur-xl group-focus-within:bg-indigo-600/10 transition-all"></div>
            <div class="relative flex items-center bg-white border-2 border-gray-100 group-focus-within:border-indigo-500 rounded-2xl p-2 transition-all shadow-sm">
              <div class="flex-1 flex items-center px-4">
                <i class="fas fa-search text-gray-400 mr-3"></i>
                <input 
                  v-model="searchQuery"
                  type="text" 
                  placeholder="Search by industry, style, or tech stack..."
                  class="w-full bg-transparent border-none outline-none text-sm font-bold text-gray-700 placeholder:text-gray-300"
                />
              </div>
              <button class="px-6 py-3 bg-gray-900 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-indigo-600 transition-all">
                Find Template
              </button>
            </div>
          </div>
    ...
    <div class="max-w-[1600px] mx-auto px-6 lg:px-12 py-12">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
        <div 
          v-for="(item, i) in filteredPortfolios" :key="i"
          class="group flex flex-col gap-5"
        >
          <div class="relative aspect-[16/10] rounded-[2.5rem] overflow-hidden border-4 border-white shadow-sm hover:shadow-2xl hover:border-indigo-50 transition-all duration-500 bg-gray-100">
            <img :src="item.src" :alt="item.label" class="w-full h-full object-cover grayscale-[40%] group-hover:grayscale-0 transition-all duration-700" />
            
            <div class="absolute top-6 right-6">
              <div v-if="item.isPro" class="h-8 w-8 rounded-xl bg-gray-900 flex items-center justify-center text-yellow-400 shadow-xl">
                <i class="fas fa-crown text-xs"></i>
              </div>
            </div>

            <div class="absolute inset-0 bg-indigo-900/80 opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col items-center justify-center p-8 text-center">
              ...
              <div class="flex items-center gap-3">
                <NuxtLink :to="`/preview/${item.id}`" class="px-6 py-3 bg-white text-gray-900 rounded-xl text-[10px] font-black uppercase tracking-widest hover:scale-105 transition-transform">
                  Live Preview
                </NuxtLink>
                <button class="h-11 w-11 rounded-xl bg-white/10 text-white hover:bg-white hover:text-indigo-600 transition-all">
      ...
      <div v-if="filteredPortfolios.length === 0" class="py-32 text-center">
        ...
        <button @click="searchQuery = ''; activeFilter = 'All'" class="text-indigo-600 font-bold text-xs mt-4 underline underline-offset-4">Reset Scanners</button>
      </div>
    </div>
  </main>
</template>
