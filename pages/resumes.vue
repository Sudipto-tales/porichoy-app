<template>
  <main class="flex-1 overflow-y-auto scroll-smooth custom-scrollbar">
    <div class="relative bg-[#fafafa] border-b border-gray-100">
      ...
      <div class="max-w-[1600px] mx-auto px-6 lg:px-12 py-20 relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-12">
        <div class="max-w-2xl">
          <div class="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-blue-600 text-white mb-8 shadow-lg shadow-blue-100">
            <span class="text-[9px] font-black uppercase tracking-[0.3em]">Precision Drafts</span>
          </div>
          <h1 class="text-5xl md:text-7xl font-black text-gray-900 tracking-tighter leading-[0.85] mb-8">
            CURATE YOUR <br />
            <span class="text-blue-600 italic">LEGACY.</span>
          </h1>
          <p class="text-gray-400 text-base md:text-lg font-medium max-w-md leading-relaxed">
            Architecturally sound resumes designed to pass through scanners and leave a lasting impression on humans.
          </p>
        </div>

        <div class="w-full max-w-md">
          <div class="bg-white p-6 rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative z-10 space-y-4">
              ...
              <div class="flex items-center border-b-2 border-gray-100 group-focus-within:border-blue-600 transition-colors pb-2">
                <input 
                  v-model="searchQuery"
                  type="text" 
              ...
              <div class="flex flex-wrap gap-2 pt-2">
                <span v-for="tag in ['Tech', 'Minimal', 'Modern']" :key="tag" class="text-[9px] font-bold px-2 py-1 bg-gray-50 text-gray-500 rounded-md cursor-pointer hover:bg-blue-50 hover:text-blue-600 transition-all">
                  #{{ tag }}
                </span>
              </div>
            </div>
      ...
      <div class="flex items-center justify-between border-b border-gray-100 mb-12">
        <div class="flex gap-10">
          <button 
            ...
            :class="['pb-4 text-[11px] font-black uppercase tracking-widest transition-all relative', 
                    activeCategory === cat ? 'text-blue-600' : 'text-gray-400 hover:text-gray-900']"
          >
            {{ cat }}
            <div v-if="activeCategory === cat" class="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600 animate-in fade-in slide-in-from-left-2"></div>
          </button>
        </div>
        
        <div class="hidden md:flex items-center gap-6">
          <span class="text-[10px] font-bold text-gray-300 uppercase tracking-widest">{{ filteredResumes.length }} Templates Found</span>
        </div>
      </div>

      <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-8">
        <div 
          v-for="(resume, i) in filteredResumes" :key="i"
          class="group space-y-4"
        >
          <div class="relative aspect-[3/4] bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden group-hover:shadow-2xl group-hover:-translate-y-2 transition-all duration-500">
            <img :src="resume.src" :alt="resume.label" class="w-full h-full object-cover" />
            
            <div v-if="resume.isPro" class="absolute top-3 left-3 px-2 py-0.5 bg-yellow-400 text-[8px] font-black uppercase rounded shadow-sm">
              Pro
            </div>

            <div class="absolute inset-0 bg-gray-900/10 opacity-0 group-hover:opacity-100 transition-all flex flex-col items-center justify-end p-6">
              <NuxtLink :to="`/builder/${resume.id}`" class="w-full py-3 bg-white text-gray-900 text-[10px] font-black uppercase tracking-widest text-center shadow-xl hover:bg-blue-600 hover:text-white transition-all">
                Use Template
              </NuxtLink>
            </div>
          </div>
...
const filteredResumes = computed(() => {
  return resumes.filter(r => {
    const matchesSearch = r.label.toLowerCase().includes(searchQuery.value.toLowerCase())
    const matchesCat = activeCategory.value === 'All Templates' || r.style === activeCategory.value
